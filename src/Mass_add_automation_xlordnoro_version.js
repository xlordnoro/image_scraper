...
async function processPost(page, postLink) {
  ...
  if (editLink) {
    ...
    const customCode = '<script type="text/javascript" src="https://xlordnoro.github.io/dynamic_loading_xlordnoro.js"></script>';
    ...
    if (customCodeExists) {
      ...
      const donateImageExists = await newPage.evaluate((selector) => {
        const textarea = document.querySelector(selector);
        return textarea && textarea.value.includes('<a class="donateImage">');
      }, textAreaSelector);
      ...
      if (donateImageExists) {
        await newPage.evaluate((selector) => {
          ...
          const indexOfDonateImage = currentContent.indexOf('<a class="donateImage">');
