...
async function processPost(browser, postLink, errorLogStream) {
  ...
  await Promise.all([
    postPage.waitForSelector('p.image img', { timeout: 60000 }), // Wait for cover image
    postPage.waitForSelector('p.donation img', { timeout: 60000 }) // Wait for donation image
  ]);

  // Extract and download cover image
  console.log('Extracting and downloading cover image...');
  await downloadImage(postPage, 'p.image img', 'cover_images');

  // Extract and download donation images
  console.log('Extracting and downloading donation images...');
  await downloadImages(postPage, 'p.donation img', 'donation_images');
